...
// 🛑 IMPORTANT: Imports assume all screen files are in the same 'screens' folder
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Row(
                  children: [
                    // GestureDetector to open the drawer
                    GestureDetector(
                      onTap: () {
            ...
            // Greeting
            Text(
              "Welcome $displayFullName!", // Dynamic Greeting
...
class AppDrawer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Drawer(
      child: Column(
        children: <Widget>[
          // DRAWER HEADER (Displays dynamic data or a loading indicator)
          Container(
...
// 🛑 IMPORTANT: You MUST ensure these files exist in your 'screens' directory.
import 'report_symptoms_screen.dart';
import 'report_risk_screen.dart';
import 'econsultation_screen.dart';
import 'map_screen.dart';
...
import 'profile_screen.dart';
import 'notifications_page.dart';
import 'login_screen.dart'; // 🛑 NEW: Required for the log-out navigation
...
class UserProfile {
  ...
  factory UserProfile.fromJson(Map<String, dynamic> json) {
    // Graceful handling for null or empty strings
    String name = json['full_name'];
    if (name == null || name.isEmpty) {
      name = "Guest User";
    }
...
class UserProvider with ChangeNotifier {
  ...
  Future<void> fetchUserProfile() async {
    ...
    try {
      ...
      // Use the correct Django profile endpoint
      final url = Uri.parse("http://localhost:8080/api/accounts/profile/");
      ...
      if (response.statusCode == 200) {
        ...
        // Clear profile if the token is invalid or the request failed
        _userProfile = null;
        print("Failed to load profile: ${response.statusCode}");
      }
    } catch (e) {
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    super.initState();
    // Trigger data fetch on load (handles app restart or login redirect)
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<UserProvider>(context, listen: false).fetchUserProfile();
    });
  }
...
// ====================================================================
// APP DRAWER IMPLEMENTATION (Includes fully functional Log Out)
// ====================================================================
      ...
      child: Column(
        children: <Widget>[
          ...
          Padding(
            ...
            child: ElevatedButton(
              onPressed: () async {
                // 🛑 LOG OUT LOGIC: Clears session and navigates to login

                // 1. Clear the stored session token (logs out everywhere)
                SharedPreferences prefs = await SharedPreferences.getInstance();
                await prefs.remove('token');

                // 2. Clear the in-app user profile state
                userProvider.clearUserProfile();

                // 3. Navigate to the Login Screen and clear all previous routes
                Navigator.of(context).pushAndRemoveUntil(
                  MaterialPageRoute(
                    // 🛑 Ensure this is the correct name for your Login screen!
